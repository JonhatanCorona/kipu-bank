...
async function main() {
  ...
  // ===============================
  // ⚡ CONFIGURA ESTO CON TUS CONTRATOS EN SEPÓLIA
  // ===============================
  const CONTRACTS = {
  ...
  const OTHER_TOKEN_TO_DEPOSIT = hre.ethers.parseEther("5"); // 50 TTK
  const MIN_USDC_TOKEN_SWAP = hre.ethers.parseUnits("1", 6);  // mínimo 1 USDC

  // ===============================
  // 🔹 Conectar contratos reales
  // ===============================
  const Bank = await hre.ethers.getContractAt("KipuBank", CONTRACTS.bank);
  const otherToken = await hre.ethers.getContractAt("IERC20", CONTRACTS.otherToken);

  // ===============================
  // 🔹 Vault antes del depósito
  // ===============================
  ...
  // ===============================
  // ✅ Aprobar TTK para KipuBank
  // ===============================
  const approveTx = await otherToken.connect(tester).approve(CONTRACTS.bank, OTHER_TOKEN_TO_DEPOSIT);
  await approveTx.wait();

  // ===============================
  // 🚀 Depositar TTK → WETH → USDC
  // ===============================
  console.log(`\n🚀 Depositando ${ethers.formatEther(OTHER_TOKEN_TO_DEPOSIT)} TOKENS a WETH para convertir a USDC...`);
  ...
  try {
    ...
    console.log("✅ Depósito TTK → WETH → USDC completado con éxito!");
  } catch (error) {
    console.error("❌ Error al depositar TTK:", error);
  }
}
